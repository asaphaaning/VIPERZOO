...
//! immutable [`viperzoo_world::snapshot::Snapshot`]s. [`Channel::spawn`]
//! schedules that owner and returns a named [`Running`] product when an
//! opinionated Tokio-owned lifecycle is desired.
//!
...
//!
//! ```text
//! Ingress ── Observation ──► Owner ──► Reducer ──► Arc<Snapshot> ──► World
//!                              │
//!                              └── Receipt ──► Ingress
//! ```
...
pub use runtime::{
    Channel, Config, Error, Ingress, Owner, Receipt, Running, SpawnError, Subscription,
    SubscriptionError, Task, TaskError, Wait, WaitError, World, channel,
};
